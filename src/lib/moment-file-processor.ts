...
export class MomentFileProcessor {
  ...
  private generateFilename(moment: PivotalMoment): string {
    ...
    // Handle date safely - extractedAt might be string from localStorage
    const extractedAtDate = typeof moment.extractedAt === 'string' 
      ? new Date(moment.extractedAt) 
      : moment.extractedAt
    const date = extractedAtDate.toISOString().split('T')[0]
  ...
  private momentToMarkdown(moment: PivotalMoment): string {
    // Safely handle date conversion - extractedAt might be string from localStorage
    const extractedAtDate = typeof moment.extractedAt === 'string' 
      ? new Date(moment.extractedAt) 
      : moment.extractedAt
    ...
    const frontmatter = {
      ...
      extractedAt: extractedAtDate.toISOString(),
      source: {
      ...
      timeline: {
        estimatedDate: moment.timeline.estimatedDate 
          ? (typeof moment.timeline.estimatedDate === 'string' 
             ? moment.timeline.estimatedDate 
             : moment.timeline.estimatedDate.toISOString())
          : undefined,
...
*This moment was automatically generated by the Moments AI Analysis Engine on ${extractedAtDate.toLocaleString()}.*
`
